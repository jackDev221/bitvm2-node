...
mod tests {
    ...
    async fn test_spv_check() {
        let global_init_config = GoatInitConfig{
            ...
            gateway_address: "0xeD8AeeD334fA446FA03Aa00B28aFf02FA8aC02df".parse().expect("parse contract address"),
        ...
        let(root, proof_info) = client.get_bitc_merkle_proof(&tx_id).await.expect("call merkle proof");
