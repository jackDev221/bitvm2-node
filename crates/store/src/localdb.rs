...
use crate::{
    ...
    MessageBroadcast, Node, NodesOverview, NonceCollect, NonceCollectMetaData, Proof, ProofWithPis,
...
impl<'a> StorageProcessor<'a> {
    ...
    pub async fn create_proof(&mut self, block_number: i64, proof: String) -> anyhow::Result<()> {
        sqlx::query!("INSERT INTO proof (block_number, proof) VALUES (?,?)", block_number, proof)
            .execute(self.conn())
            .await?;
        Ok(())
    }
