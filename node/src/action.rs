...
pub async fn store_committee_pub_nonces(
    ...
    let nonces_arr:[String; COMMITTEE_PRE_SIGN_NUM] = nonces_vec.try_into().map_err(|v:Vec<String>|format!("length wrong: expect {}, real {}", COMMITTEE_PRE_SIGN_NUM, v.len()))?;
    Ok(storage_process.store_nonces(
        instance_id,
        graph_id,
        &vec![nonces_arr],
    ...
    match storage_process.get_nonces(instance_id, graph_id).await?{
        ...
        Some(nonce_collect) => {
            ...
            for nonces_item in nonce_collect.nonces{
                let nonce_vec:Vec<PubNonce> = nonces_item.iter().map(|v|PubNonce::from_str(v).expect("fail to decode pub nonce")).collect();
                res.push(nonce_vec.try_into().map_err(|v:Vec<PubNonce>|format!("length wrong: expect {}, real {}", COMMITTEE_PRE_SIGN_NUM, v.len()))?)
            }
            Ok(
                res
            )
...
/// Q: committee_pubkey will change?
...
pub async  fn store_graph(
    ...
    let mut storage_process = client.local_db.acquire().await?;
    let assert_commit_txids :Vec<String>= graph.assert_commit.commit_txns
        .iter()
        .map(
            |v|v.tx().compute_txid().to_string()
        ).collect();
    storage_process.update_graph(Graph{
        graph_id,
        instance_id,
...
pub async  fn update_graph(
    ...
    store_graph(client, instance_id, graph_id, graph_state, graph).await
}
...
pub async fn get_graph(
    ...
    if graph.raw_data.is_none(){
        return Err(
            format!(
                "grap with graph_id:{} raw data is none",
