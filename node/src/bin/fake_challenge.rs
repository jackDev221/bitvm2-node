...
async fn main() {
    ...
    let kickoff_txid =
        get_tx_from_ipfs(&client, &base_url, IpfsTxName::Kickoff).await.unwrap().compute_txid();
    if !tx_on_chain(&client, &kickoff_txid).await.unwrap() {
        println!("Error: kickoff tx not broadcasted!");
        return;
    }
    if !outpoint_available(&client, &kickoff_txid, 1).await.unwrap() {
        println!("Error: challenge/take1 already sent!");
        return;
    }
    let challenge_tx = get_tx_from_ipfs(&client, &base_url, IpfsTxName::Challenge).await.unwrap();
    let challenge_txid =
        complete_and_broadcast_challenge_tx(&client, master_key.master_keypair(), challenge_tx)
            .await
            .unwrap();
