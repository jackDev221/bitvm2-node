...
pub async fn scan_kickoff(
    ...
    for graph_data in graph_datas {
        ...
        if let Some(_) = outpoint_spent_txid(btc_client, &kickoff_txid, 1).await? {
            tracing::trace!(
...
pub async fn scan_assert(
    ...
    for graph_data in graphs {
        ...
        if let Some(_) = outpoint_spent_txid(btc_client, &assert_final_txid, 1).await? {
            tracing::trace!(
...
pub async fn scan_take1(
    ...
    for graph_data in graph_datas {
        ...
        if graph_data.graph_status == GraphStatus::Challenge.to_string()
            && is_need_to_send_msg(graph_data.msg_times, graph_data.last_msg_send_at)
        {
