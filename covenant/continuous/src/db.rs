...
impl ExecutionHooks for PersistToDB {
    async fn on_execution_start(&self, block_number: u64) -> eyre::Result<()> {
        let mut storage_process = self.local_db.acquire().await.map_err(|e| {
            eyre!("Failed to acquire local db: {e}")
        })?;

        storage_process.update_block_executing(
            block_number as i64,
            ProvableBlockStatus::Queued.to_string(),
        ).await.map_err(|e| {
            eyre!("Failed to start block execution: {e}")
        })?;
    ...
    async fn on_execution_end<P: NodePrimitives>(
        ...
        storage_process.update_block_executed(
            ...
            eyre!("Failed to end block execution: {e}")
        })?;
    ...
    async fn on_proving_end(
        ...
        storage_process.update_block_proved(
            block_number as i64,
            ...
            eyre!("Failed to end block proving: {e}")
        })?;
...
pub async fn task_failed(local_db: Arc<LocalDB>, block_number: u64, err: String) -> eyre::Result<()> {
    let mut storage_process = local_db.acquire().await.map_err(|e| {
        eyre!("Failed to acquire local db: {e}")
    })?;

    storage_process.update_block_proving_failed(
        block_number as i64,
        ProvableBlockStatus::Failed.to_string(),
        err,
    ).await.map_err(|e| {
        eyre!("Failed to mark task as failed: {e}")
    })?;
